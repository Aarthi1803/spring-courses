...
public class SecurityConfig {
	...
	public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception { // SecurityFilterChain: It defines security rules (authentication & authorization). HttpSecurity http: This is Spring Security’s main configuration object, used to define security policies.
		...
		http.csrf(customizer -> customizer.disable()) // Disables CSRF (Cross-Site Request Forgery) protection. CSRF prevents unauthorized form submissions but is unnecessary for APIs that don’t use sessions. If you are building a REST API (e.g., Mobile App Backend), CSRF is not needed because users don’t submit forms. If you are building a banking website, DO NOT disable CSRF (because CSRF protection prevents unauthorized fund transfers).
			.authorizeHttpRequests(request -> 
				request.requestMatchers("register", "login") // request for register should be directly accessed without login details
						.permitAll() 
						.anyRequest().authenticated()) // Requires authentication for all requests (every API needs login). This ensures that only logged-in users can access any endpoint.
			.sessionManagement(session -> session.sessionCreationPolicy(SessionCreationPolicy.STATELESS)); // Disables server-side sessions (stateless authentication). Every request is independent (server does not store user sessions).
	...
	@Bean // Creates a Bean for Authentication Manager, so Spring Security can use it.
...
public class SecurityConfig { // This configures security rules, allowing some APIs to be public and securing others.
	
	@Autowired
	private UserDetailsService userDetailsService; // Injects an UserDetailsService instance (used to fetch user details from the database). Spring will automatically inject MyUserDetailsService as it implements UserDetailsService interface - class MyUserDetailsService class implements UserDetailsService
	
	@Autowired
	private JwtFilter jwtFilter; // Injects JwtFilter, which checks JWT tokens in each request. If the token is valid, the user is authenticated automatically.
	
	@Bean
	public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception { // SecurityFilterChain: It defines security rules (authentication & authorization). HttpSecurity http: This is Spring Security’s main configuration object, used to define security policies.

		http.csrf(customizer -> customizer.disable()) // Disables CSRF (Cross-Site Request Forgery) protection. CSRF prevents unauthorized form submissions but is unnecessary for APIs that don’t use sessions.  CSRF is needed only for web apps with form-based authentication, but not for REST APIs. If you are building a REST API (e.g., Mobile App Backend), CSRF is not needed because users don’t submit forms. If you are building a banking website, DO NOT disable CSRF (because CSRF protection prevents unauthorized fund transfers).
			.authorizeHttpRequests(request -> 
				request.requestMatchers("register", "login").permitAll()  // request for /register and /login should be directly accessed without credentials. Allows public access to register and login.
						.anyRequest().authenticated()) // Requires authentication for all requests (every API needs login). This ensures that only logged-in users can access any endpoint.
			.sessionManagement(session -> session.sessionCreationPolicy(SessionCreationPolicy.STATELESS)) // Disables server-side sessions (stateless authentication). Every request is independent (server does not store user sessions) and requires a valid JWT token.
			.addFilterBefore(jwtFilter,UsernamePasswordAuthenticationFilter.class); // Adds the JWT filter before Spring Security’s default UsernamePasswordAuthenticationFilter.
	...
	@Bean // Creates a Bean for Authentication Provider, so Spring Security can use it.
	public AuthenticationProvider authProvider() { // implements authentication by verifying users from a database instead of in-memory users. Defines authentication logic (loads user details from the database).
		
		DaoAuthenticationProvider provider = new DaoAuthenticationProvider(); //DAO = Data Access Object. Used to verify user credentials from the database instead of in-memory. To verify user from database we have used DaoAuthenticationProvider
		provider.setUserDetailsService(userDetailsService); // Tells DaoAuthenticationProvider to use MyUserDetailsService to fetch users details.
		provider.setPasswordEncoder(new BCryptPasswordEncoder(12)); // Encrypts passwords before saving by creating password as 2^12. Ensure that authentication works with encrypted passwords. During login, BCrypt automatically matches encrypted passwords. Encrypts passwords using BCrypt before saving them to the database.  During login, passwords are automatically checked using encryption.
	...
	@Bean // Creates a Bean for Authentication Manager, so Spring Security can use it.
	public AuthenticationManager authenticationManager(AuthenticationConfiguration config) throws Exception { // Creates an authentication manager to handle login. Used by Spring Security when users log in.
		return config.getAuthenticationManager();
	}
}
...
This class is responsible for configuring security rules in a Spring Boot application using Spring Security.

✔ It defines which APIs are protected and which are public.
✔ It integrates JWT (JSON Web Token) authentication for securing APIs.
✔ It configures authentication logic (verifying users from the database).

How This Works in a Real Application:
1️. User Registers (POST /register)
    The password is hashed & stored in the database.

2️. User Logs In (POST /login)
    The username & password are verified.
    If valid, a JWT token is generated.

3️. User Accesses Secure API (GET /secure-data)
